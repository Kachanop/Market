...
export default function MarketManage({ markets, setMarkets }) {
  ...
  // State สำหรับสร้างตลาดใหม่หรือแก้ไขตลาดเดิม
  ...
  // Interaction (การโต้ตอบกับ Canvas)
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      // กด ESC เพื่อยกเลิกการวาด หรือยกเลิกการเลือก
      if (e.key === 'Escape') {
        if (isDrawing) {
          setIsDrawing(false);
          setCurrentPoints([]);
      ...
      // กด Ctrl+D เพื่อคัดลอกล็อกที่เลือกอยู่
  ...
  // ฟังก์ชันย่อขนาดรูปภาพ (Resize Image)
  ...
  // ฟังก์ชันสร้างเส้น Path แบบโค้งมน (Smooth Path) จากจุดพิกัด
  ...
  // --- Handlers (การจัดการเหตุการณ์ต่างๆ) ---
  ...
  // เริ่มวาดรูปทรงใหม่ (สี่เหลี่ยม หรือ วงกลม)
  const addShape = (type) => {
  ...
  // จัดการคลิกที่ตัวล็อกเพื่อเลือกแก้ไข
  ...
  // จัดการคลิกที่จุด (Point) เพื่อแก้ไขรูปร่าง
  ...
  // บันทึกข้อมูลล็อกลง state หลัก
  const handleSaveLock = () => {
    if (!editingLock) return;
    // ✅ บันทึกข้อมูล zone ด้วย
    const lockData = {
      ...editingLock,
    ...
    delete lockData.tenant; delete lockData.status; delete lockData.description; delete lockData.isNew;
  ...
  // อัปเดตความโค้งของจุด (Bezier curve tension)
  ...
  const styles = {
    ...
    centerPanel: { flex: 1, position: 'relative', overflow: 'hidden', background: '#E5E5EA', display: 'flex', justifyContent: 'center', alignItems: 'center' },
    ...
    input: { width: '100%', padding: '10px', border: '1px solid #E5E5EA', borderRadius: '10px', fontSize: '0.9rem', background: '#F2F2F7', outline: 'none', boxSizing: 'border-box' },
    textarea: { width: '100%', padding: '10px', border: '1px solid #E5E5EA', borderRadius: '10px', fontSize: '0.9rem', background: '#F2F2F7', outline: 'none', resize: 'vertical', minHeight: '60px', boxSizing: 'border-box' },
    ...
    editInput: { flex: 1, padding: '6px 8px', border: '1px solid #007AFF', borderRadius: '8px', fontSize: '0.9rem', marginBottom: '5px', boxSizing: 'border-box' },
    editTextarea: { width: '100%', padding: '6px 8px', border: '1px solid #007AFF', borderRadius: '8px', fontSize: '0.85rem', marginBottom: '10px', minHeight: '50px', resize: 'vertical', boxSizing: 'border-box' },
      ...
      <div style={styles.rightPanel}>
        {editingLock ? (
          <div style={{ padding: '20px', overflowY: 'auto' }}>
            ...
            {/* ✅ เพิ่มช่อง Zone ตรงนี้ */}
            <div style={{ marginBottom: '15px' }}>
              <label style={{ fontSize: '0.8rem', color: '#8E8E93', fontWeight: '600' }}>Zone</label>
              <input
                style={styles.input}
                value={editingLock.zone || ''}
                onChange={e => setEditingLock({ ...editingLock, zone: e.target.value })}
                placeholder="Ex. Zone A, Food Court"
              />
            </div>
